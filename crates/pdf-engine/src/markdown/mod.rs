...
fn convert_items_with_rects_lines_and_table_output(
    ...
    // Recover vertical fractions (numerator / rule / denominator) before any
    // downstream stage: the numerator must lose its rule-caused `<u>` flag,
    // and the denominator items are absorbed into the numerator text so line
    // grouping keeps the equation on one line instead of stranding the
    // denominator below the surrounding paragraph.

    let (fraction_edits, fraction_dropped, fraction_y_snaps) =
        fractions::plan_inline_fractions(&items, rects, pdf_lines);
    let mut items = items;
    for (idx, text) in fraction_edits {
        if let Some(item) = items.get_mut(idx) {
            item.text = text;
            item.is_underline = false;
        }
    }
    for (idx, y) in fraction_y_snaps {
        if let Some(item) = items.get_mut(idx) {
            item.y = y;
        }
    }
    ...
    for (input_index, item) in items.into_iter().enumerate() {
        if fraction_dropped.contains(&input_index) {
            continue;
        }
    ...
    // logical chart-page position as tables before reinsertion. Embedded
    // raster figures have no recoverable source URL, so emit a stable
    // numbered marker the caller can map to extracted assets if needed.
    let mut page_images: HashMap<u32, Vec<PositionedMarkdown>> = HashMap::new();
    let mut image_seq: usize = 0;
    let mut ordered_images: Vec<&TextItem> = images.iter().collect();
    ordered_images.sort_by(|a, b| {
        a.page
            .cmp(&b.page)
    ...
    for img in ordered_images {
        image_seq += 1;
        // When the extraction pass resolved this figure to an embedded
        // image object, its text was rewritten to the asset file name
        // (`image-3.jpg`) — use it as the marker URL so links resolve.
        let url = if img.text.starts_with("image-") {
            img.text.clone()
        } else {
            format!("image-{image_seq}")
        };
        let img_md = format!("![图片{image_seq}]({url})\n");
        page_images
            .entry(img.page)
            .or_default()
            .push(PositionedMarkdown::new(
...
mod tests {
    ...
    fn chart_prose_order_survives_physical_gutter_detection() {
        ...
        // Image markers carry pre-assigned numbers in (page, top-down, x)
        // order: the right figure (y=650) is 图片1, the left one (y=600) is
        // 图片2. Column-order emission must therefore put 图片2 first.
        let left_image = markdown.find("图片2").unwrap();
        let right_image = markdown.find("图片1").unwrap();
        assert!(
