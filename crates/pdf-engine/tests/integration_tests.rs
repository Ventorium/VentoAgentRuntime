...
use vento_pdf_engine::detector::{estimate_page_count_from_bytes, DetectionConfig, ScanStrategy};
use vento_pdf_engine::extractor::group_into_lines;
use vento_pdf_engine::types::ItemType;
use vento_pdf_engine::types::TextLine;
use vento_pdf_engine::{
...
fn make_text_item_with_font(
    ...
    use vento_pdf_engine::extractor::{is_bold_font, is_italic_font, ItemType};
    TextItem {
        text: text.to_string(),
        x,
        y,
...
fn test_markdown_options_default() {
    ...
    assert_eq!(opts.profile, vento_pdf_engine::MarkdownProfile::Fidelity);
...
fn test_markdown_options_custom() {
    let opts = MarkdownOptions {
        profile: vento_pdf_engine::MarkdownProfile::Compact,
    ...
    assert!(!opts.detect_headers);
    assert_eq!(opts.profile, vento_pdf_engine::MarkdownProfile::Compact);
    assert!(opts.detect_lists);
    assert!(!opts.detect_code);
...
fn test_markdown_from_items_empty() {
    use vento_pdf_engine::markdown::to_markdown_from_items;
    let items: Vec<TextItem> = vec![];
...
fn test_markdown_from_lines_empty() {
    use vento_pdf_engine::markdown::to_markdown_from_lines;
...
fn test_not_a_pdf_html_input() {
    ...
    let result = vento_pdf_engine::process_pdf_mem(html);
...
fn test_not_a_pdf_xml_input() {
    ...
    let result = vento_pdf_engine::process_pdf_mem(xml);
...
fn test_not_a_pdf_json_input() {
    ...
    let result = vento_pdf_engine::process_pdf_mem(json);
...
fn test_not_a_pdf_plain_text_input() {
    ...
    let result = vento_pdf_engine::process_pdf_mem(text);
    assert_not_a_pdf(result, "plain text");
}

#[test]
fn test_not_a_pdf_empty_buffer() {
    let result = vento_pdf_engine::process_pdf_mem(b"");
...
fn test_bom_prefixed_pdf_header_not_rejected() {
    ...
    let result = vento_pdf_engine::process_pdf_mem(&bom_pdf);
    if let Err(PdfError::NotAPdf(_)) = result {
...
fn test_extract_text_mem_uses_container_repair() {
    ...
    let text = vento_pdf_engine::extractor::extract_text_mem(&pdf)
...
fn test_not_a_pdf_detect_pdf_type_mem() {
    ...
    let result = vento_pdf_engine::detector::detect_pdf_type_mem(html);
    assert_not_a_pdf(result, "HTML");
}
...
fn test_not_a_pdf_extract_text_with_positions_mem() {
    ...
    let result = vento_pdf_engine::extractor::extract_text_with_positions_mem(html);
    assert_not_a_pdf(result, "HTML");
}
...
fn test_not_a_pdf_extract_text_mem() {
    ...
    let result = vento_pdf_engine::extractor::extract_text_mem(xml);
    assert_not_a_pdf(result, "XML");
}
...
fn assert_snapshot(fixture: &str) -> String {
    ...
    let result = vento_pdf_engine::process_pdf(&fixture_path)
...
fn test_pages_needing_ocr_field_accessible() {
    ...
    let detection_result = vento_pdf_engine::detector::PdfTypeResult {
    ...
    let process_result = vento_pdf_engine::PdfProcessResult {
        pdf_type: PdfType::TextBased,
        markdown: None,
        images: Vec::new(),
        page_count: 1,
        ...
        title: None,
        confidence: 1.0,
        layout: vento_pdf_engine::LayoutComplexity::default(),
...
300
%%EOF";
    let result = vento_pdf_engine::process_pdf_mem(pdf_bytes);
...
fn test_firecrawl_tagged_pdf_struct_tree() {
    use lopdf::Document;
    use vento_pdf_engine::structure_tree::{StructRole, StructTree};
    ...
    let result = vento_pdf_engine::process_pdf_mem(&buf).unwrap();
...
fn test_tagged_pdf_text_items_carry_mcid() {
    let buf = std::fs::read("tests/fixtures/firecrawl_docs_tagged.pdf").unwrap();
    let items = vento_pdf_engine::extractor::extract_text_with_positions_mem(&buf).unwrap();
    assert!(
...
fn test_extract_structure_elements_tagged_pdf() {
    let buf = std::fs::read("tests/fixtures/firecrawl_docs_tagged.pdf").unwrap();
    let elements = vento_pdf_engine::extract_structure_elements_mem(&buf, None).unwrap();
    ...
    let page1 = vento_pdf_engine::extract_structure_elements_mem(&buf, Some(&[1])).unwrap();
...
fn test_collect_text_in_region_keeps_partial_overlap_items() {
    ...
    let text = vento_pdf_engine::collect_text_in_region(&[item], 95.0, 80.0, 120.0, 110.0, 800.0);
    assert!(
...
fn test_collect_text_in_region_uses_rtl_sorting() {
    ...
    let text = vento_pdf_engine::collect_text_in_region(&items, 0.0, 0.0, 600.0, 800.0, 800.0);
    assert_eq!(
        text, "مرحبا بكم",
...
fn test_detect_vector_grid_in_region_line_pdf() {
    use vento_pdf_engine::{extract_tables_with_structure_mem, TsrTableInput};
...
fn test_extract_tables_with_structure_not_a_pdf() {
    use vento_pdf_engine::extract_tables_with_structure_mem;
...
fn test_extract_tables_with_structure_cells_real_pdf_bits_pilani() {
    use vento_pdf_engine::{extract_tables_with_structure_cells_mem, TsrTableInput};
...
fn test_auto_passes_through_clean_tsr_output() {
    use vento_pdf_engine::{extract_tables_with_structure_auto_mem, TsrTableInput};
...
fn test_auto_returns_empty_inputs() {
    use vento_pdf_engine::extract_tables_with_structure_auto_mem;
    let buf = synthetic_dense_table_pdf();
...
fn test_extract_pages_markdown_ocrs_page_with_vector_outlined_text() {
    ...
    let cls =
        vento_pdf_engine::detector::detect_pdf_type_mem(&buf).expect("fixture should classify");
    assert!(
