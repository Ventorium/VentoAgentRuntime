...
pub(super) fn parse(pkg: &mut Package, wb_part: &str) -> Result<Document, ConvertError> {
    ...
    for (name, part) in &sheets {
        let content = pkg
            .optional_part(part)?
            .map(|bytes| read_sheet(&bytes, &shared, &xfs, date1904));
    ...
    while let Some((id, payload)) = records.next()? {
        ...
        match id {
            ...
            BRT_MERGE_CELL => {
                ...
                if r1 != r2 || c1 != c2 {
                    out.merges
                        .push((r1.min(r2), c1.min(c2), r1.max(r2), c1.max(c2)));
                }
            }
...
mod tests {
    ...
    fn styles(fmts: &[(u16, &str)], xf_fmt_ids: &[u16]) -> Vec<u8> {
        ...
        out.extend(rec(
            BRT_BEGIN_CELL_XFS,
            &(xf_fmt_ids.len() as u32).to_le_bytes(),
        ));
    ...
    fn truncated_record_stream_is_malformed() {
        ...
        assert!(
            records.next().is_err(),
            "payload shorter than its declared size must error"
        );
    }
    ...
    fn strings_resolve_from_the_shared_table_and_inline() {
        ...
        let wb = Wb {
            shared: Some(shared_strings(&["zeroth", "shared"])),
            ..one_sheet(body)
        };
        let doc = parse(&wb.build()).unwrap();
    ...
    fn hidden_rows_columns_and_sheets_are_omitted() {
        ...
        assert_eq!(
            doc.blocks.len(),
            1,
            "hidden sheets must add no heading and no table"
        );
        assert_eq!(
            texts(first_table(&doc)),
            vec![vec!["1", "3"], vec!["5", ""]]
