...
fn convert(path: &Path) -> String {
    let result = std::panic::catch_unwind(|| vento_file_parser::to_markdown(path));
    match result {
        Ok(Ok(md)) => md,
        ...
        if let Some(outcome) = expected_outcome(&path) {
            ...
            match outcome {
                "errors" => {
                    assert!(
                        is_err,
                        "{name}: annotated `errors` but converted successfully"
                    )
                }
                ...
                _ if STRICT_ANNOTATIONS => {
                    assert!(
                        !is_err,
                        "{name}: annotated `{outcome}` but failed: {output}"
                    )
                }
                _ => {}
            }
...
fn scanned_pages_are_reported_not_dropped() {
    ...
    match vento_file_parser::to_markdown_bytes(&mixed, vento_file_parser::Format::Pdf) {
        Err(vento_file_parser::ConvertError::NeedsOcr { pages, page_count }) => {
...
fn embedded_ole_payload_is_retained() {
    ...
    let bytes = std::fs::read(&path).unwrap();
    let doc = vento_file_parser::to_document(&bytes, vento_file_parser::Format::Pptx).unwrap();
    let ole = doc
        .assets
        .iter()
...
fn docx_ole_payload_wins_over_its_preview_image() {
    ...
    let bytes = std::fs::read(&path).unwrap();
    let doc = vento_file_parser::to_document(&bytes, vento_file_parser::Format::Docx).unwrap();
    let ole = doc
        .assets
        .iter()
        .find(|a| a.media_type == "application/vnd.ms-ole-object")
...
fn repeated_part_references_convert_and_dedupe() {
    ...
    assert_eq!(
        doc.assets.len(),
        1,
        "one shared asset for seventy references"
    );
}
...
fn doc_inline_picture_is_retained() {
    ...
    let doc = vento_file_parser::to_document(&bytes, vento_file_parser::Format::Doc).unwrap();
    assert!(
        doc.assets
            .iter()
            .any(|a| a.media_type.starts_with("image/")),
        "expected an extracted picture asset, got {:?}",
        doc.assets
            .iter()
            .map(|a| (&a.media_type, a.bytes.len()))
            .collect::<Vec<_>>()
    );
}
...
fn rtf_inline_picture_is_retained() {
    ...
    let doc = vento_file_parser::to_document(&bytes, vento_file_parser::Format::Rtf).unwrap();
...
fn abuse_fixtures_hard_fail() {
    ...
    for path in files {
        ...
        let result = vento_file_parser::to_markdown(&path);
        assert!(
            matches!(
                result,
                Err(vento_file_parser::ConvertError::ResourceLimit { .. })
            ),
...
fn digest(output: &str) -> String {
    ...
    let hash: String = Sha256::digest(output.as_bytes())
        .iter()
        .map(|b| format!("{b:02x}"))
        .collect();
